...
**Re:** Unauthenticated-by-misconfig ClearML server, `195.251.122.130`
**Severity:** HIGH
...
The host at `195.251.122.130` runs ClearML version `(via HTTPS)` with `basic.enabled=true`. ClearML is an open-source MLOps platform. With basic-auth signup enabled, anyone on the internet can register an account against the operator's training-experiment workspace. Effectively unauthenticated access for any visitor who registers.

Operator note: this host belongs to Alexander Technological Educational Institute, Thessaloniki.
