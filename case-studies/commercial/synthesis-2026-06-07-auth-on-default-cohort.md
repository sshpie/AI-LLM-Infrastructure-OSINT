---
type: synthesis
title: "The Auth-on-Default Landscape of OSS AI/LLM Infrastructure"
date: 2026-06-07
summary: "Two-day population survey across 13 OSS AI/LLM infrastructure platforms reveals a maintainer-culture-axis split between demo-first defaults (auth-permissive, 70-91% open) and enterprise-customer-first defaults (auth-required, 0-1%). The cohort is not jurisdiction-defined. Insight #76 scope-bounded to platform class; LLM02 Sensitive Information Disclosure is the dominant finding class; the Capitol.ai escalation demonstrates the maintainer-default failing at enterprise-SaaS scale; in-flight attacker /proc/self/environ activity directly observable on OpenHands instances."
tags: [synthesis, auth-on-default, insight-76, cohort-analysis, maintainer-culture, owasp-llm-top-10, capitol-ai, openhands, librechat, langfuse, ragflow, phoenix, herald]
featured: true
abstract: "A new generation of open-source AI and LLM infrastructure tools has shipped over the past 24 months, and many of them ship with auth-permissive defaults. Those defaults survive into production deployment at university research labs, enterprise customer-tenant fleets, financial-messaging companies, mental-health nonprofits, and national education ministries. Across 13 platform surveys over two days, the rate at which the default holds correlates strongly with upstream maintainer culture — specifically, whether the maintainer team optimizes for 'clone and demo it immediately' or 'self-host for your enterprise customer.' The rate is not jurisdiction-specific. It is movable in 2–3 minor-version cycles by upstream pressure, including internal-quality-driven pressure. The class of finding most directly enabled is OWASP LLM02:2025 Sensitive Information Disclosure (promoted #6→#2 in the 2025 revision specifically because of incidents of this type), with LLM06 Excessive Agency and LLM10 Unbounded Consumption as the next-most-common consequences."
---
...
SNI-correct probing of all four customer-tenant Langfuse instances confirmed `signUpDisabled: false` (v3.155.1 / v3.157.0). Each customer-dedicated Langfuse instance ships with the Langfuse upstream default unchanged. A non-customer can register on the customer-tenant instance and potentially access trace data showing the EY or HMG users' LLM activity (depending on Langfuse workspace isolation).
...
Harvard University, Arizona State University, UC Santa Barbara, UC Berkeley Civil & Environmental Engineering, Northeastern University (Phoenix with "Essaybot" project), Hong Kong University of Science and Technology, Brno University of Technology (Czechia), Indiana University, Khajeh Nasir Toosi University of Technology (Iran — OFAC-sensitive), Shenzhen Middle School (K-12), Taiwan Ministry of Education Computer Center (**3 separate findings same day** consolidated for TWCERT/CC).

**Healthcare, mental health, and sensitive data:**
Santé Pair (French mental-health peer-support nonprofit — GDPR Article 9 special-category health data), Strategion GmbH (kardiointerakt medical AI with open LiteLLM proxy to Claude Opus 4.7), Inspirali AI DEV (medical education DEV environment).

**Legal:**
TruslerLegal AI Assistant ("Better Divorce Austin" — Texas family-law boutique with SERVER_KEY OpenAI + Anthropic), LegalMatch AI (production properly Cloudflare-fronted, MVP environment exposed), Atticus Legal Assistant, Legal-Knowledge-Graph-Chatbot.

**Government and national infrastructure:**
PLLuM dla Edukacji (Polish National Research Institute / NASK state cybersecurity authority), SwiftRef Assistant (SWIFT financial messaging reference data), Singular GovTech Singapore, SENAI Brazil (national vocational education, ~3M students/year — LGPD-class), Capitol.ai (suspected UK HMG and Ernst & Young customer-tenant fleet).

**Enterprise / commercial:**
deepset|PepsiCo, Allwyn UK National Lottery (3 IPs), Groupe Narbonne (French auto parts retail), PromoPharma AI, Capitol AI Chat Agent 20-host fleet, multiple "smaller commercial" branded deployments.
